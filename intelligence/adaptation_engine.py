...
class AdaptationEngine:
    ...
    def apply(self, state, risk_engine):
        ...
        realized_values = [float(item.get("realized_pnl", 0.0)) for item in sample_trades]
        ...
        if state.total_drawdown_pct > (0.5 * risk_engine.max_daily_dd) or expectancy_realized < 0:
            target = current * 0.9
