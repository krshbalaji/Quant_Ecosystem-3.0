...
from quant_ecosystem.core.persistence.outcome_memory import OutcomeMemory
...
from quant_ecosystem.strategy_bank.engine.strategy_bank_engine import StrategyBankEngine
from quant_ecosystem.strategy_bank.live_strategy_engine import LiveStrategyEngine
from quant_ecosystem.strategy_bank.mutation.mutation_engine import MutationEngine
from quant_ecosystem.strategy_bank.strategy_registry import StrategyRegistry
from quant_ecosystem.portfolio.alpha_competition_engine import AlphaCompetitionEngine
from quant_ecosystem.core.capital.capital_intelligence_engine import CapitalIntelligenceEngine
from quant_ecosystem.research.alpha_evolution_engine import AlphaEvolutionEngine
...
from quant_ecosystem.evolution.capital_intelligence_engine import CapitalIntelligenceEngine
from quant_ecosystem.evolution.meta_alpha_engine import MetaAlphaEngine
...
def build_router(config):
    ...
    router.capital_ai = capital_ai
    router.meta_alpha = meta_alpha
    ...
    capital_ai = CapitalIntelligenceEngine(portfolio_engine)
    meta_alpha = MetaAlphaEngine(strategy_registry)
    ...
    # create broker router
    broker = BrokerRouter(config)
    ...
    from quant_ecosystem.research.alpha_competition_engine import AlphaCompetitionEngine
    alpha_competition = AlphaCompetitionEngine(
        strategy_registry=strategy_registry
    )

    # Attach engine to router
    router.alpha_competition = alpha_competition

    
    broker_router = BrokerRouter(config)

    strategy_registry = StrategyRegistry()

    strategy_engine = StrategyBankEngine(
        strategy_registry=strategy_registry
    )
    ...
    alpha_evolution = AlphaEvolutionEngine(
        strategy_registry=strategy_registry,
        mutation_engine=mutation_engine,
        crossover_engine=crossover_engine,
        evaluation_engine=strategy_evaluator,
    )
