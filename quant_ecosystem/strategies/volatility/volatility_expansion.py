...
class VolatilityExpansionStrategy(BaseStrategy):
    ...
    def generate_signal(self, market_data) -> Optional[Signal]:
        ...
        expansion_value = self.params.get("expansion_factor", 1.5)
        expansion_threshold = float(
            expansion_value
            if isinstance(expansion_value, (int, float))
            else 1.5
        )

        sl_value = self.params.get("stop_loss_pct", 1.2)
        stop_loss_pct = float(
            sl_value
