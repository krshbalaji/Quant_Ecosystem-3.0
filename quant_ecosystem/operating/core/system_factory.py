...
class SystemRouter:
    ...
    * All engine attrs accessed via getattr(router, "...", None)
    ...
    def __init__(self, config: Any) -> None:
        ...
        self.data_layer:       Optional[Any] = None
        self.symbols:          list          = []
        ...
        self.pre_market_intelligence: Optional[Any] = None
        self.event_intelligence:      Optional[Any] = None
        self.day_planner:             Optional[Any] = None
        self.self_learning_engine:    Optional[Any] = None
        self.eod_report_engine:       Optional[Any] = None
    ...
    async def execute(self, signal: Any = None, market_bias: str = "NEUTRAL",
        ...
        self._execution_router.symbols = list(self.symbols or [])
    ...
    def start_trading(self) -> str:
        if self._execution_router is not None and hasattr(self._execution_router, "start_trading"):
            return self._execution_router.start_trading()
        if self.state is not None:
    ...
    def kill_switch(self) -> str:
        if self._execution_router is not None and hasattr(self._execution_router, "kill_switch"):
            return self._execution_router.kill_switch()
        if self.state is not None:
    ...
    def set_trading_mode(self, mode: str) -> str:
        ...
        if self._execution_router is not None and hasattr(self._execution_router, "set_trading_mode"):
            return self._execution_router.set_trading_mode(normalized)
        return f"Mode set to {normalized}"

    def set_risk_preset(self, preset: str) -> str:
        if self._execution_router is not None and hasattr(self._execution_router, "set_risk_preset"):
            return self._execution_router.set_risk_preset(preset)
        return f"Risk preset {preset} applied."

    def set_strategy_profile(self, profile: str) -> str:
        if self._execution_router is not None and hasattr(self._execution_router, "set_strategy_profile"):
            return self._execution_router.set_strategy_profile(profile)
        return f"Strategy profile set to {str(profile).upper()}."

    def get_status_report(self) -> str:
        if self._execution_router is not None and hasattr(self._execution_router, "get_status_report"):
            return self._execution_router.get_status_report()
        return "System status unavailable."

    def get_positions_report(self) -> str:
        if self._execution_router is not None and hasattr(self._execution_router, "get_positions_report"):
            return self._execution_router.get_positions_report()
        return "Open positions: 0"

    def get_strategy_report(self) -> str:
        if self._execution_router is not None and hasattr(self._execution_router, "get_strategy_report"):
            return self._execution_router.get_strategy_report()
        return "Active strategies: none"

    def get_dashboard_report(self) -> str:
        if self._execution_router is not None and hasattr(self._execution_router, "get_dashboard_report"):
            return self._execution_router.get_dashboard_report()
        return "Institutional Control Terminal\n\nDashboard report unavailable."
    ...
    def _log_mode_summary(self, mode: OperatingMode) -> None:
        active_engines = [
            name for name in (
                ...
                "data_layer", "strategy_manager", "watchdog", "memory_engine",
            )
...
class SystemFactory:
    ...
    def build(self) -> SystemRouter:
        ...
        logger.info(
            "Production hardening context | environment=%s data_mode=%s",
            str(getattr(self._config, "environment", "DEV")).upper(),
            str(getattr(self._config, "data_mode", "SYNTHETIC")).upper(),
        )
        if self._mode is OperatingMode.LIVE:
            live_enabled = bool(getattr(self._config, "enable_live_trading", False))
            strict_live = bool(getattr(self._config, "strict_live_safeguards", True))
            if strict_live and not live_enabled:
                raise RuntimeError("LIVE trading not enabled. Set ENABLE_LIVE_TRADING=true")

        router = SystemRouter(config=self._config)

        self._boot_core_layer(router)           # 1. core
        self._boot_production_hardening(router)
        ...
        self._boot_decision_support_layer(router)
    ...
    def _boot_core_layer(self, router: SystemRouter) -> None:
        ...
        # System state
        try:
            from quant_ecosystem.operating.core.system_state import SystemState  # noqa: PLC0415
            router.state = SystemState()
            router.state.trading_mode = self._mode.value
            router.state.environment = str(getattr(self._config, "environment", "DEV")).upper()
            router.state.data_mode = str(getattr(self._config, "data_mode", "SYNTHETIC")).upper()
        ...
        universe = None
        try:
            from quant_ecosystem.operating.market.market_universe_manager import (  # noqa: PLC0415
        ...
        # Market data engine
        from quant_ecosystem.operating.market.market_data_engine import MarketDataEngine

        market_data_engine = MarketDataEngine()
        market_data_engine.start()

        router.market_data = market_data_engine

        try:
            from quant_ecosystem.operating.market.market_data_engine import (  # noqa: PLC0415
                MarketDataEngine,
            )
        ...
        # Risk engine
        try:
            from quant_ecosystem.operating.risk.risk_engine import RiskEngine  # noqa: PLC0415
        ...
        # Portfolio engine
        try:
            from quant_ecosystem.operating.portfolio.portfolio_engine import (  # noqa: PLC0415
                PortfolioEngine,
            )
        ...
        try:
            from quant_ecosystem.operating.core.persistence.outcome_memory import (  # noqa: PLC0415
                OutcomeMemory,
            )
    ...
    def _boot_research_memory(self, router: SystemRouter) -> None:
        ...
        try:
            from quant_ecosystem.research.evolution.alpha_evolution_engine import (  # noqa: PLC0415
        ...
        try:
            from quant_ecosystem.operating.portfolio.capital_intelligence_engine import (  # noqa: PLC0415
    ...
    def _boot_genome_memory(self, router: SystemRouter) -> None:
        ...
        try:
            ...
            try:
                from quant_ecosystem.research.alpha_genome.genome_library import (  # noqa: PLC0415
                    AlphaGenomeLibrary,
                )
        ...
        if not getattr(cfg, "enable_alpha_genome_engine", False):
            return
        try:
            from quant_ecosystem.research.alpha_genome.genome_generator import (  # noqa: PLC0415
                GenomeGenerator,
            )
        ...
        try:
            from quant_ecosystem.research.alpha_genome.genome_evaluator import (  # noqa: PLC0415
                GenomeEvaluator,
            )
    ...
    def _boot_synthetic_market(self, router: SystemRouter) -> None:
        ...
        # FORCE synthetic engine for development (FIX: no market data issue)

        try:
            from quant_ecosystem.operating.market.synthetic_market_engine import SyntheticMarketEngine

            logger.info("[boot] synthetic_market FORCED (DEV MODE)")

            synthetic = SyntheticMarketEngine()
            synthetic.start()

            router.synthetic_market = synthetic

        except Exception as e:
            logger.warning(f"SyntheticMarketEngine failed: {e}")
    ...
    def _boot_meta_research(self, router: SystemRouter) -> None:
        ...
        try:
            from quant_ecosystem.research.meta_research import MetaResearchAI  # noqa: PLC0415
    ...
    def _boot_alpha_factory(self, router: SystemRouter) -> None:
        ...
        try:
            from quant_ecosystem.research.alpha_factory.factory_controller import (  # noqa: PLC0415
                FactoryController,
            )
    ...
    def _boot_autonomous_lab(self, router: SystemRouter) -> None:
        ...
        try:
            from quant_ecosystem.research.strategy_lab.lab_controller import (  # noqa: PLC0415
    ...
    def _boot_execution_layer(self, router: SystemRouter) -> None:
        ...
        try:
            from quant_ecosystem.operating.broker.fyers_broker import FyersBroker  # noqa: PLC0415
        ...
        try:
            from quant_ecosystem.operating.broker.broker_router import BrokerRouter  # noqa: PLC0415
        ...
        try:
            from quant_ecosystem.operating.broker.reconciliation.broker_reconciler import BrokerReconciler
        ...
        try:
            from quant_ecosystem.operating.strategy_bank.governance.portfolio_capital_governor import PortfolioCapitalGovernor
        ...
        try:
            from quant_ecosystem.operating.execution.execution_router import ExecutionRouter

            er = ExecutionRouter(
                broker=getattr(router, "_broker_router", None),
                risk_engine=router.risk_engine,
                state=router.state,
                runtime_state=getattr(router, 'runtime_state', None),
                market_data=router.market_data,
                config=self._config,
                data_layer=router.data_layer,
            ...
            self._register_runtime_brokers(router, er)
    ...
    def _register_runtime_brokers(self, router: SystemRouter, execution_router: Any) -> None:
        """Register all available broker backends for runtime failover."""
        try:
            if getattr(router, "_broker", None) is not None:
                execution_router.register_broker("fyers", router._broker)
        except Exception:
            logger.warning("Fyers broker registration failed.", exc_info=True)

        try:
            from quant_ecosystem.operating.broker.groww_broker import GrowwBroker  # noqa: PLC0415

            groww = GrowwBroker(config=self._config)
            groww.connect()
            execution_router.register_broker("groww", groww)
            setattr(router, "_groww_broker", groww)
        except Exception:
            logger.warning("Groww broker unavailable for runtime registration.", exc_info=True)

        try:
            from quant_ecosystem.operating.broker.coinswitch_broker import CoinSwitchBroker  # noqa: PLC0415

            coinswitch = CoinSwitchBroker()
            coinswitch.connect()
            execution_router.register_broker("coinswitch", coinswitch)
            setattr(router, "_coinswitch_broker", coinswitch)
        except Exception:
            logger.warning("CoinSwitch broker unavailable for runtime registration.", exc_info=True)
    ...
    def _boot_live_broker(self, router: SystemRouter) -> None:
        ...
        try:
            if broker_name == "FYERS":
                ...
                from quant_ecosystem.operating.broker.coinswitch_broker import (  # noqa: PLC0415
                    CoinSwitchBroker,
                )
                ...
                live_broker = GrowwBroker(config=self._config)
            else:
    ...
    def _boot_strategy_layer(self, router: SystemRouter) -> None:
        ...
        # Strategy registry
        from quant_ecosystem.operating.strategy_bank.engine.strategy_registry import StrategyRegistry
        from quant_ecosystem.operating.strategy_bank.live_strategy_engine import LiveStrategyEngine
    ...
    def _boot_strategy_bank(self, router: SystemRouter) -> None:
        ...
        try:
            from quant_ecosystem.operating.strategy_bank.engine.strategy_bank_engine import (  # noqa: PLC0415
                StrategyBankEngine,
            )
        ...
        try:
            from quant_ecosystem.operating.strategy_bank.layer import (  # noqa: PLC0415
                StrategyBankLayer,
            )
            ...
            from quant_ecosystem.research.strategy_governance.lifecycle_parliament_engine import LifecycleParliamentEngine
            router.lifecycle_parliament = LifecycleParliamentEngine()

        try:
            from quant_ecosystem.operating.strategy_selector.selector_core import (  # noqa: PLC0415
                SelectorCore,
            )
            ...
            from quant_ecosystem.operating.strategy_selector.selector_bridge import SelectorBridge
            from quant_ecosystem.operating.strategy_selector.selector_core import SelectorCore

            selector = SelectorCore()

            router.selector_bridge = SelectorBridge(
        ...
        try:
            from quant_ecosystem.research.capital_allocator.allocation_engine import (  # noqa: PLC0415
                AllocationEngine,
            )
    ...
    def _boot_meta_strategy_brain(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.meta_strategy.meta_brain import MetaBrain  # noqa: PLC0415
    ...
    def _boot_strategy_diversity(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.strategy_diversity.diversity_engine import (  # noqa: PLC0415
                DiversityEngine,
            )
    ...
    def _boot_strategy_survival(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.strategy_survival.survival_engine import (  # noqa: PLC0415
                SurvivalEngine,
            )
    ...
    def _boot_alpha_scanner(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.alpha_scanner.scanner_core import (  # noqa: PLC0415
                AlphaScannerCore,
            )
    ...
    def _boot_portfolio_ai(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.operating.portfolio_ai.portfolio_ai_core import (  # noqa: PLC0415
                PortfolioAICore,
            )
    ...
    def _boot_shadow_trading(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.shadow_trading.shadow_engine import (  # noqa: PLC0415
                ShadowEngine,
            )
    ...
    def _boot_intelligence_layer(self, router: SystemRouter) -> None:
        ...
        try:
            from quant_ecosystem.research.autonomous_controller.controller import (  # noqa: PLC0415
        ...
        # Event bus
        try:
            from quant_ecosystem.operating.core.event_bus import EventBus  # noqa: PLC0415
        ...
        try:
            from quant_ecosystem.operating.market_regime.regime_detector import (  # noqa: PLC0415
                RegimeDetector,
            )
        ...
        # Global intelligence engine (injected with market_data). End-to-end signal quality enrichment.
        try:
            from quant_ecosystem.operating.intelligence.global_intelligence_engine import (  # noqa: PLC0415
                GlobalIntelligenceEngine,
            )
            router.global_intelligence_engine = GlobalIntelligenceEngine(
                config=self._config,
                market_data=router.market_data,
            )
            logger.debug("GlobalIntelligenceEngine initialized and market_data injected.")
        except Exception:
            logger.warning("GlobalIntelligenceEngine unavailable (non-critical).", exc_info=True)

        from quant_ecosystem.operating.intelligence.resolution_registry import ResolutionRegistry

        router.resolution_registry = ResolutionRegistry()

        logger.info(
            "[boot] ResolutionRegistry initialized | horizons=%s",
            list(router.resolution_registry._resolutions.keys()),
        )

    def _boot_regime_ai(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.regime_ai.regime_ai_core import (  # noqa: PLC0415
                RegimeAICore,
            )
    ...
    def _boot_event_signal_engine(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.event_signal_engine.event_driven_signal_engine import (  # noqa: PLC0415
    ...
    def _boot_market_pulse(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.operating.market_pulse.pulse_engine import (  # noqa: PLC0415
                PulseEngine,
            )
    ...
    def _boot_event_driven_orchestrator(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.event_engine.event_orchestrator import (  # noqa: PLC0415
                EventOrchestrator,
            )
    ...
    def _boot_adaptive_learning(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.adaptive_learning.learning_engine import (  # noqa: PLC0415
                LearningEngine,
            )
    ...
    def _boot_cognitive_control(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.cognitive_control.cognitive_controller import (  # noqa: PLC0415
                CognitiveController,
            )
    ...
    def _boot_global_market_brain(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.research.global_market_brain.market_brain import (  # noqa: PLC0415
                MarketBrain,
            )
    ...
    def _boot_decision_support_layer(self, router: SystemRouter) -> None:
        try:
            from quant_ecosystem.operating.intelligence.pre_market_intelligence_engine import PreMarketIntelligenceEngine  # noqa: PLC0415
            router.pre_market_intelligence = PreMarketIntelligenceEngine(config=self._config)
        except Exception:
            logger.warning("PreMarketIntelligenceEngine unavailable.", exc_info=True)
        try:
            from quant_ecosystem.operating.intelligence.event_intelligence_engine import EventIntelligenceEngine  # noqa: PLC0415
            router.event_intelligence = EventIntelligenceEngine(config=self._config)
        except Exception:
            logger.warning("EventIntelligenceEngine unavailable.", exc_info=True)
        try:
            from quant_ecosystem.operating.intelligence.day_planner import DayPlanner  # noqa: PLC0415
            router.day_planner = DayPlanner(config=self._config)
        except Exception:
            logger.warning("DayPlanner unavailable.", exc_info=True)
        try:
            from quant_ecosystem.operating.intelligence.self_learning_engine import SelfLearningEngine  # noqa: PLC0415
            router.self_learning_engine = SelfLearningEngine(config=self._config)
        except Exception:
            logger.warning("SelfLearningEngine unavailable.", exc_info=True)
        try:
            from quant_ecosystem.operating.reporting.eod_report_engine import EODReportEngine  # noqa: PLC0415
            router.eod_report_engine = EODReportEngine(output_dir="storage")
        except Exception:
            logger.warning("EODReportEngine unavailable.", exc_info=True)

    def _boot_production_hardening(self, router: SystemRouter) -> None:
        logger.info("[boot] production hardening layer …")
        try:
            from quant_ecosystem.research.data_layer.data_layer import DataLayer  # noqa: PLC0415

            router.data_layer = DataLayer(config=self._config, market_data=router.market_data)
            logger.debug("DataLayer initialized.")
        except Exception:
            logger.warning("DataLayer unavailable.", exc_info=True)

        try:
            from quant_ecosystem.operating.strategy_bank.strategy_manager import StrategyManager  # noqa: PLC0415

            router.strategy_manager = StrategyManager()
            logger.debug("StrategyManager initialized.")
        except Exception:
            logger.warning("StrategyManager unavailable.", exc_info=True)

        try:
            from quant_ecosystem.operating.core.system_watchdog import SystemWatchdog  # noqa: PLC0415

            router.watchdog = SystemWatchdog(
                freeze_timeout_sec=float(getattr(self._config, "watchdog_freeze_timeout_sec", 20.0)),
                latency_warn_ms=float(getattr(self._config, "watchdog_latency_warn_ms", 1500.0)),
            )
            logger.debug("SystemWatchdog initialized.")
        except Exception:
            logger.warning("SystemWatchdog unavailable.", exc_info=True)

        try:
            from quant_ecosystem.operating.core.memory_engine import MemoryEngine  # noqa: PLC0415

            router.memory_engine = MemoryEngine(root="storage/memory")
            logger.debug("MemoryEngine initialized.")
        except Exception:
            logger.warning("MemoryEngine unavailable.", exc_info=True)
    ...
    def _boot_safety_layer(self, router: SystemRouter) -> None:
        ...
        try:
            from quant_ecosystem.research.safety_governor.governor_core import (  # noqa: PLC0415
                GovernorCore,
            )
    ...
    def _boot_communication_layer(self, router: SystemRouter) -> None:
        ...
        try:
            from quant_ecosystem.operating.control.telegram_controller import (  # noqa: PLC0415
                TelegramController,
            )
    ...
    def _boot_autonomous_research(self, router: SystemRouter) -> None:
            ...
            from quant_ecosystem.operating.intelligence.multi_resolution_orchestrator import (
                MultiResolutionResearchOrchestrator,
            )
            ...
            if resolution_registry is None:
                logger.warning(
                    "[boot] ResolutionRegistry not found — institutional research disabled"
                )
                return

            orchestrator = MultiResolutionResearchOrchestrator(
                router=router,
                resolution_registry=resolution_registry
            )

            router.institutional_orchestrator = orchestrator
...
def _make_grid_result_callback(router):
    ...
    def _on_result(result) -> None:
        ...
        print("REAL THRESHOLD =", getattr(router.research_grid, "promote_threshold", None))
        ...
        if result.fitness >= thr and genome_id:
            ...
            if lib is not None:
                try:
                    ...
                    registry = getattr(router, "strategy_registry", None)
