...
class Config:
    ...
    def __init__(self, **kwargs):
        ...
        self.groww_api_secret = os.getenv("GROWW_API_SECRET", "")
        self.groww_enable_live = os.getenv("GROWW_ENABLE_LIVE", "false").lower() == "true"
        ...
        self.telegram_cycle_summary_interval = int(os.getenv("TELEGRAM_CYCLE_SUMMARY_INTERVAL", "5"))
        self.telegram_viewer_ids = self._parse_ids("TELEGRAM_VIEWER_IDS")
        ...
        self.enable_dashboard_auth = os.getenv("ENABLE_DASHBOARD_AUTH", "false").lower() == "true"
        self.dashboard_admin_username = os.getenv("DASHBOARD_ADMIN_USERNAME", "admin")
        self.dashboard_admin_password = os.getenv("DASHBOARD_ADMIN_PASSWORD", "")
        self.dashboard_viewer_username = os.getenv("DASHBOARD_VIEWER_USERNAME", "viewer")
        self.dashboard_viewer_password = os.getenv("DASHBOARD_VIEWER_PASSWORD", "")
        ...
        # Mode
        self.environment = os.getenv("ENVIRONMENT", os.getenv("APP_ENV", "DEV")).upper()
        self.mode = os.getenv("MODE", "PAPER").upper()
        self.data_mode = os.getenv(
            "DATA_MODE",
            "SYNTHETIC" if self.environment == "DEV" else ("LIVE" if self.mode == "LIVE" else "PAPER"),
        ).upper()
        self.enable_live_trading = os.getenv("ENABLE_LIVE_TRADING", "false").lower() == "true"
        self.strict_live_safeguards = os.getenv("STRICT_LIVE_SAFEGUARDS", "true").lower() == "true"
        self.operation_mode = os.getenv("OPERATION_MODE", "AUTONOMOUS")
        self.auto_start = os.getenv("AUTO_START", "false").lower() == "true"
        self.runtime_cycles = int(os.getenv("RUNTIME_CYCLES", "0"))
        ...
        self.execution_retry_attempts = int(os.getenv("EXECUTION_RETRY_ATTEMPTS", "2"))
        self.execution_retry_delay_ms = int(os.getenv("EXECUTION_RETRY_DELAY_MS", "250"))
        self.watchdog_freeze_timeout_sec = float(os.getenv("WATCHDOG_FREEZE_TIMEOUT_SEC", "20"))
        self.watchdog_latency_warn_ms = float(os.getenv("WATCHDOG_LATENCY_WARN_MS", "1500"))
