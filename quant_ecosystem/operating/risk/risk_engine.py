...
class RiskEngine:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if config is None:
            try:
                from quant_ecosystem.operating.core.config_loader import Config  # noqa: lazy
                config = Config()
        ...
        self.max_symbol_exposure = self.max_symbol_risk
        self.max_portfolio_heat = 50.0
    ...
    def check_order(self, order: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if realized < 0 and abs(realized) >= (self._get_equity() * (self.max_daily_dd / 100.0)):
            if hasattr(state, "trading_halted"):
                state.trading_halted = True
        ...
        if not exposure_reducing:
            ...
            if port_pct >= self.max_portfolio_heat:
                return False, "PORTFOLIO_HEAT_LIMIT"
            sym_pct = self._compute_symbol_exposure(symbol, price)
            if sym_pct >= min(self.max_symbol_risk, self.max_symbol_exposure):
    ...
    def evaluate(self, signal: Dict[str, Any]) -> bool:
        allowed, _reason = self.check_order(signal)
        return allowed
